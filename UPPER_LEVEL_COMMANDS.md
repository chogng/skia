...
| 隔离图层 | `save_layer` / `restore` | `save_layer` / `restore` | `save_layer` / `restore`（software reference 已执行；Metal 待补） |
...
| 当前实际硬件后端 | CPU 可用 | CPU 可用 | Metal 支持 `clear`、SourceOver `FillRect` / `FillPath` / `StrokePath`、atlas glyph batch 与复杂裁剪；渐变/滤镜/图层/更多混合模式正在后端补齐；Vulkan 骨架支持真实离屏 `clear`/readback |
...
- GPU 命令层没有专用 `draw_glyph_run` 或文字装饰命令；`skia-gpu-text` 已可把矢量 glyph 降为
  ...
  generic GPU 和 Metal 支持的 draw 路径。
...
1. gradient、color filter 和 save-layer 已进入共享 paint/command 模型并由 CPU/software reference
   执行；Metal 仍需执行这些语义并补齐 `SourceOver` 之外的混合模式；
