...
| 当前实际硬件后端 | CPU 可用 | CPU 可用 | Metal 支持 `clear`、SourceOver `FillRect`、atlas glyph batch 与复杂裁剪；Vulkan 骨架支持真实离屏 `clear`/readback |
...
| `clip_path(path, rule, op)` | 按 `EvenOdd` 或 `NonZero` 和当前裁剪求交/差。 | 曲线复用 fill 的固定步数展平；mask 为每像素一字节。 |
...
- `MetalBackend` 会真实执行 `Clear`，把 `FillRect` 展开为两个 triangle，并上传 glyph atlas 后
  ...
  glyph。复杂裁剪按被引用的 `GpuClipId` 父链生成临时 R8 texture：每个节点在一次 submit 中
  只生成一次，子节点采样父 mask，`FillRect` 与 glyph fragment shader 再采样最终 mask；Path
  曲线使用与 CPU 相同的固定 16 段展平。矩形和文字路径目前仅支持 `SourceOver`，遇到
  `FillPath`、`StrokePath`、`DrawImage` 或其他 blend mode 会返回 `UnsupportedCommand`。
...
2. GPU 命令层缺少矢量文字，bitmap 文字需预建 atlas；实线文字装饰已由 adapter 转成
   `FillRect`，但波浪线、虚线等变体仍未实现；
3. Metal 的非清屏能力目前只有 SourceOver 矩形与 atlas glyph batch，仍缺 fill/stroke path 和 image；
4. CPU Canvas、DisplayList、generic GPU 和 Metal 已支持矩形与路径的交集/差集裁剪；Metal 目前只把它应用到已支持的矩形/glyph draw，图片仍是 RGBA8；
...
`cpu/src/canvas.rs`，复杂裁剪 mask 在 `cpu/src/clip.rs`，描边分段与命中算法在 `cpu/src/stroke.rs`。文字仍由单一 `skia-text` crate 负责：基础 glyph/run 类型在
...
后端提交在 `gpu/metal/src/lib.rs`，Metal mask 生命周期在 `gpu/metal/src/clip.rs`，裁剪边生成与
曲线展平在 `gpu/metal/src/clip_geometry.rs`，mask 与 draw shader 在 `shaders/solid_rect.metal`。
