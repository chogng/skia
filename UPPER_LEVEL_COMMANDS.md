...
| 裁剪 | `clip_rect`、`clip_rect_with_op`、`clip_path` | `clip_rect`、`clip_rect_with_op`、`clip_path` | `clip_rect` |
...
| `StrokePath` / `stroke_path` | `GpuPathId`、`StrokeOptions`、`Paint` | 描边已登记路径，并快照 transform 与 target-space scissor。 |
...
- `MetalBackend` 会真实执行 `Clear`，把 `FillRect` 展开为两个 triangle，并上传 glyph atlas 后
  ...
  glyph。矩形和文字路径目前仅支持 `SourceOver`，遇到 `FillPath`、`StrokePath`、`DrawImage`
  或其他 blend mode 会返回 `UnsupportedCommand`。
...
4. CPU Canvas/DisplayList 已支持矩形与路径的交集/差集裁剪；GPU/Metal 的复杂裁剪仍待接入，图片仍是 RGBA8；
