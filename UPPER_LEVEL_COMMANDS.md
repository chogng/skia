...
`GpuTextAtlasBuilder::new(width, height, max_glyphs)`，随后调用
`insert_layout(&layout, &collection)` 并 `finish()`。再以
`atlas.register(&mut encoder)` 把 atlas 移入 command buffer，使用返回的
`GpuTextAtlasRegistration::draw_layout(&layout, origin, paint)` 生成按 layout 最终
位置排列的单批 glyph quad。该方法只绘制 glyph，不绘制 underline/strike-through，装饰仍需
另录普通几何命令。

atlas 使用 1 像素 padding、以 `(FontId, GlyphId, font_size_bits, GlyphBitmapFormat)` 去重，并受
尺寸和 glyph 数上限约束；空间不足会明确返回 `ResourceLimit`。registration 同时持有 text
索引、准确的 `GpuGlyphAtlasId` 和所属 encoder 的独占借用，调用方不能把一个 atlas 的字形坐标
误配给另一个 command buffer；需要穿插状态或非文字命令时通过 `registration.encoder()` 访问
同一个 encoder，完成后 drop registration 再 `finish()`。通用
...
| 绘制文字 | `draw_glyph_run`、`draw_shaped_paragraph`、`draw_text_layout` | `draw_glyph_run`（paragraph/layout 需展开） | `GpuTextAtlasRegistration::draw_layout`（adapter 生成 atlas glyph batch） |
...
`TextLayout` 到 `DrawGlyphs` 的转换不属于 GPU encoder：它只存在于 `skia-gpu-text`，通过
`GpuTextAtlasBuilder` → `GpuTextAtlas::register` → `GpuTextAtlasRegistration::draw_layout` 完成。
...
`core/src/display_list.rs`，GPU 命令层在 `gpu/src/lib.rs`，GPU 文字适配在
`gpu/text/src/lib.rs`，Metal 后端在 `gpu/metal/src/lib.rs`。
...
device 时硬件用例会跳过，shader library 创建或编译异常则直接失败。最后一条 dependency tree
