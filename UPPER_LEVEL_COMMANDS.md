...
| 当前实际硬件后端 | CPU 可用 | CPU 可用 | Metal 支持 `clear`、全部 `BlendMode`、渐变/颜色滤镜/box blur 隔离图层、图片、path/stroke、atlas glyph batch 与复杂裁剪；Vulkan 骨架支持真实离屏 `clear`/readback |
...
本轮上层缺口的收口状态如下；其中 Vulkan 与有损 WebP 按当前分工明确留给独立实现，不属于
本轮继续改动范围：
...
2. Vulkan 当前只实现真实 offscreen clear/readback，fill/path/image/text、复杂裁剪与 swapchain
   需要完整 shader/descriptor/synchronization 子系统；本轮不改；
