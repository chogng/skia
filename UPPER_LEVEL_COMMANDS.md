...
格式实现保持在 `skia-codec` 内部，并归入 API 私有模块 `codec/src/api/png.rs`、
`codec/src/api/jpeg.rs`、`codec/src/api/webp.rs`。PNG 和 WebP 经 `image` 分别使用纯 Rust `png` 与
...
源码入口：Geometry 在 `geometry/src/lib.rs`，Path 实现在 `path/src/path.rs` 并由薄入口
`path/src/lib.rs` 导出，CPU Canvas 入口在
...
`effects/src/path_effect.rs`，而可扩展 `PathEffect` 契约与资源上限位于
`core/src/path_effect.rs`。文字仍由单一 `skia-text` crate 负责：基础 glyph/run 类型在
...
DisplayList 在 `core/src/display_list.rs`，GPU 命令层在 `gpu/src/command.rs` 并由薄入口
`gpu/src/lib.rs` 导出，GPU 文字适配 package 位于
