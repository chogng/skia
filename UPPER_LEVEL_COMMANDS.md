...
Solid/Dashed/Dotted/Wavy per-span underline/strike-through、GDEF ligature 内部 caret 和轮廓解析**，
但不负责平台字体发现、generic family 映射、variable 实例选择策略、语言偏好或内置词典/断字算法。
...
- GPU 命令层没有矢量 `draw_glyph_run` 或专用文字装饰命令；bitmap text 必须先建立 atlas，
  四种 underline/strike-through 线型均由 `skia-gpu-text` 降为普通 `FillRect`。它仍没有渐变、滤镜、
...
1. GPU 命令层缺少矢量文字，bitmap 文字需预建 atlas；Solid/Dashed/Dotted/Wavy 文字装饰已由
   adapter 转成普通 `FillRect` batch；
...
4. 图片已支持 Nearest/Linear 过滤，CPU 图片仍只接受轴对齐变换；描边已支持闭合轮廓的 Center/Inside/Outside 对齐；
5. 文本层已有内存字体解析、family/style 匹配、单段落 bidi、跨字体 fallback、metrics、
   通用换行、可插拔词典断点、hyphenation、对齐、Unicode 空格/CJK inter-character
   justification 与 Solid/Dashed/Dotted/Wavy 装饰，
   ...
   variable 实例选择策略、通用跨脚本 inter-character justification 与完整排版；
