...
| `stroke_path_with_options(path, options, paint)` | 完整描边路径。 | 复用 `skia-tessellation` 的固定步数曲线展平；支持 butt/round/square cap、miter/round/bevel join、miter limit，以及偶数正值 on/off dash pattern 和规范化 phase；暂不支持内/外描边对齐。 |
| `stroke_to_path(path, options, transform)` | 将描边扩展为普通 `Path`。 | facade 函数；以指定 affine transform 展平并生成可用 `FillRule::NonZero` 填充的确定性三角形路径，适合复用到裁剪、缓存或后续 path 流程。 |
...
| `apply_path_effect(path, effect, transform, limits)` | 通过统一 `PathEffect` 接口执行一个路径效果。 | trim/corner/discrete 已实现该可扩展接口；效果移除全部几何时返回 `None`，每次执行都受 `PathEffectLimits` 约束。 |
...
| `draw_image(image, destination, opacity, blend_mode)` | 以兼容默认值绘制 RGBA8 图片。 | 等价于 `SamplingOptions::NEAREST`；`opacity` 只乘源 alpha；CPU 仍拒绝旋转或错切变换。 |
| `draw_image_with_sampling(image, destination, opacity, blend_mode, sampling)` | 以显式采样配置绘制 RGBA8 图片。 | 支持 texel-center、clamp-to-edge 的 Nearest/Linear；CPU 用确定性整数双线性插值且仅接受轴对齐变换。 |
...
- 命令本身不携带“当时状态”的快照；回放时按命令顺序维护状态。因此保存/恢复顺序是
  列表语义的一部分。
- 回放使用 Canvas，所以最终约束与 CPU Canvas 一致，包括完整裁剪状态、图片轴对齐限制、
  描边样式和文字轮廓解析要求。
...
| `StrokePath` / `stroke_path` | `GpuPathId`、`StrokeOptions`、`Paint` | 描边已登记路径，并快照 transform、scissor 与复杂裁剪 ID。 |
| `DrawImage` / `draw_image`、`draw_image_with_sampling` | `GpuImageId`、目标 `Rect`、`u8` opacity、`BlendMode`、`SamplingOptions` | 兼容入口记录 Nearest；显式入口快照 Nearest/Linear 采样语义。 |
...
- GPU 命令层没有矢量 `draw_glyph_run` 或专用文字装饰命令；bitmap text 必须先建立 atlas，
  实线 underline/strike-through 由 `skia-gpu-text` 降为普通 `FillRect`。它仍没有渐变、滤镜、
  图层；复杂裁剪已进入 generic GPU 和 Metal 支持的 draw 路径。
...
- `MetalBackend` 会真实执行 `Clear`，把 `FillRect` 展开为两个 triangle，并上传 glyph atlas 后
  ...
  两种 glyph。`DrawImage` 以 Nearest 或 Linear 采样 RGBA8 图片，支持任意仿射变换、opacity、scissor 与复杂
  裁剪。`StrokePath` 复用共享的描边三角形并由 Metal 光栅化为 R8 覆盖 mask，再进行最终 blend 与复杂裁剪；
  当前矩形、路径、描边、文字和图片仅支持 `SourceOver`，其他 blend mode 会返回
  `UnsupportedCommand`。
- `VulkanBackend` 动态加载系统 Vulkan loader，选择 graphics queue，创建 optimal-tiled RGBA8
  image，并用 transfer clear + staging buffer 完成精确像素读回。当前除 `Clear` 外的命令全部
  fail-closed；没有 CPU fallback，也尚未创建 swapchain、shader/descriptor pipeline 或复杂裁剪 mask。
...
1. GPU 命令层缺少矢量文字，bitmap 文字需预建 atlas；实线文字装饰已由 adapter 转成
   `FillRect`，但波浪线、虚线等变体仍未实现；
2. Metal 的非清屏能力支持 SourceOver 矩形、路径 fill/描边、RGBA8 图片与 atlas glyph batch；描边
   由共享层展开为三角形并在 GPU 光栅化为 R8 mask；
3. CPU Canvas、DisplayList、generic GPU 和 Metal 已支持矩形与路径的交集/差集裁剪；Metal 会把复杂裁剪应用到已支持的矩形、路径 fill/描边、RGBA8 图片和 glyph draw；
4. 图片已支持 Nearest/Linear 过滤；CPU 图片仍只接受轴对齐变换，描边暂不支持内/外对齐；
5. 文本层已有内存字体解析、family/style 匹配、单段落 bidi、跨字体 fallback、metrics、
   ...
   波浪线/虚线装饰变体与完整排版；
6. `path_boolean`、`stroke_to_path`、统一可组合的 `PathEffect`、`trim_path`、`corner_path` 与 deterministic `discrete_path` 已提供；其他 path effects 尚未暴露。
