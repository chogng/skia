...
`TextAtlasBuilder` → `TextAtlas::layout_quads` → generic atlas registration/batch draw 完成。
...
`core/src/display_list.rs`，GPU 命令层在 `gpu/src/lib.rs`，GPU 文字适配 package 位于
`gpu/text/`（atlas packing 与 layout quad 转换分别在 `src/atlas.rs`、`src/layout.rs`），Metal
