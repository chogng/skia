...
3. CPU Canvas、DisplayList、generic GPU 和 Metal 已支持矩形与路径的交集/差集裁剪；Metal 目前只把它应用到已支持的矩形/glyph draw，图片仍是 RGBA8；
...
5. 文本层已有内存字体解析、family/style 匹配、单段落 bidi、跨字体 fallback、metrics、
   ...
   波浪线/虚线装饰变体与完整排版；
6. 路径的几何布尔运算和 path effects 尚未暴露；它们不能由像素混合模式替代。`stroke_to_path`
   已提供为普通路径转换入口。
...
`cpu/src/stroke.rs`。文字仍由单一 `skia-text` crate 负责：基础 glyph/run 类型在
