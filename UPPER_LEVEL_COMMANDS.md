...
当前剩余项已不是 CPU/DisplayList/GPU text 之间的基础语义断层，而是需要新增依赖、数据或
渲染子系统的边界：

1. Metal 当前绘制只实现 `SourceOver`，generic paint 尚无 gradient、filter 或 save-layer 模型；
2. Vulkan 当前只实现真实 offscreen clear/readback，fill/path/image/text、复杂裁剪与 swapchain
   需要完整 shader/descriptor/synchronization 子系统；
3. 系统字体发现、generic-family 映射和语言字体偏好需要独立平台 adapter 与目录/缓存策略，
   不应重新耦合进纯 Rust `skia-text`；
4. 内置断字词典需要选定语言覆盖、词典数据来源、版本与许可证；现有 `TextBreakProvider`
   已提供稳定注入边界；
5. 当前纯 Rust WebP encoder 明确只支持 VP8L；有损 WebP 需要引入 native `libwebp`，或等待/
   选择新的纯 Rust lossy encoder；
6. `path_boolean`、`stroke_to_path`、统一可组合的 `PathEffect`、`trim_path`、`corner_path` 与
   deterministic `discrete_path` 已提供；其他 path effects 属于后续 API 扩展而非现有后端不一致。
