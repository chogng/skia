use skia_core::{Point, Scalar, SkiaError, SkiaErrorCode, StrokeCap, StrokeJoin, StrokeOptions};

use crate::canvas::{Contour, DeviceRect, ceil_q16_i64, contour_bounds, floor_q16_i64};

#[derive(Debug)]
pub(crate) struct StrokePiece {
    points: Vec<Point>,
    closed: bool,
}

pub(crate) fn stroke_pieces(
    contours: &[Contour],
    ...
    for contour in contours {
        ...
        if options.dash_pattern().is_empty() {
            ...
            let dashed = dash_contour(&points, contour.closed, options)?;
            pieces
...
fn normalized_stroke_points(contour: &Contour) -> Result<Vec<Point>, SkiaError> {
...
fn dash_contour(
    ...
    for index in 0..segment_count {
        ...
        let length = segment_length_bits(start, end)?;
        if length == 0 {
            continue;
        }
        ...
        while offset < length {
            ...
            if pattern_index.is_multiple_of(2) {
                push_unique_point(
                    &mut current,
                    interpolate_segment(start, end, offset, length)?,
                )?;
                push_unique_point(
                    &mut current,
                    interpolate_segment(start, end, offset + step, length)?,
                )?;
            }
            offset += step;
...
fn segment_length_bits(start: Point, end: Point) -> Result<i64, SkiaError> {
...
pub(crate) fn stroke_contains(
    ...
    for piece in pieces {
        ...
        let segment_count = if piece.closed {
            piece.points.len()
        } else {
            piece.points.len() - 1
        };
        for index in 0..segment_count {
            let start = piece.points[index];
            let end = piece.points[(index + 1) % piece.points.len()];
            let square_start = !piece.closed && index == 0 && options.cap() == StrokeCap::Square;
            let square_end =
                !piece.closed && index + 1 == segment_count && options.cap() == StrokeCap::Square;
        ...
        if !piece.closed
            ...
            && (point_near_point(sample, piece.points[0], options.width())?
                || point_near_point(
                    sample,
        ...
        if piece.closed {
            for index in 0..piece.points.len() {
                let previous = piece.points[(index + piece.points.len() - 1) % piece.points.len()];
                let vertex = piece.points[index];
                let next = piece.points[(index + 1) % piece.points.len()];
                if stroke_join_contains(sample, previous, vertex, next, options)? {
                    return Ok(true);
                }
            }
        } else {
            for index in 1..piece.points.len() - 1 {
...
fn point_in_segment_strip(
    ...
    let projection = relative_x
        ...
        .ok_or(SkiaError::new(SkiaErrorCode::NumericOverflow))?;
    let length = i128::from(segment_length_bits(start, end)?);
